...
form_window  = uic.loadUiType('./lol_recommendation.ui')[0]
...
class Exam(QWidget, form_window):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.Tfidf_matrix = mmread('./models/Tfidf_lol_review.mtx').tocsr()
        with open('./models/tfidf.pickle','rb') as f:
            self.Tfidf = pickle.load(f)
        self.embedding_model = Word2Vec.load('./models/word2vec_lol_review.model')
        self.df_reviews = pd.read_csv('./crawling_data/cleaned_review.csv')
